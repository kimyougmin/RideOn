...
const item = ref(itemStore.selectedItem);
...
watch(() => route.path, (newPath) => {
  if (newPath === "/detail" && !item.value) {
    ...
    if (savedItem) {
      ...
      itemStore.setSelectedItem(item.value);
    }
  }
});
</script>
  ...
  <div class="bg-black2 pt-[37px] pb-[100px] dark:bg-black8">
    <div class="w-[1044px] mx-auto">
      <div class="flex pb-[100px]">
        <img :src="item.image" class="border rounded-lg w-[514px] h-[514px]"/>
        <div class="p-4 grid grid-cols-1 content-between">
          ...
          <div>
            <p class="font-impact text-right text-3xl dark:text-black1">{{ item.lprice ? Number(item.lprice).toLocaleString("ko-KR") + "원" : "가격 없음" }}</p>
