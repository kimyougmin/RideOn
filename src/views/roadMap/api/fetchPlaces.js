...
export async function fetchPlaces(map, userLatLng, updateStationData) {
  ...
  try {
    ...
    const stations = await fetchData(stationUrl, 7);  // 6032개 데이터를 모두 가져오기 위해 7페이지 요청
    ...
    if (userLatLng) {
      ...
      stations.splice(100);  // 100개까지만 표시
    } else {
      stations.splice(100);  // 100개까지만 표시
    }

    // 마커와 클릭 이벤트를 추가하여 마커 클릭 시 해당 정보를 부모 컴포넌트로 전달
    stations.forEach((station) => {
      ...
      const customIcon = L.divIcon({
        className: 'custom-icon', // custom class
        html: `
      ...
      // 마커 클릭 시 해당 대여소 정보를 부모 컴포넌트로 전달
      marker.on('click', () => {
        updateStationData({
          stationName,
