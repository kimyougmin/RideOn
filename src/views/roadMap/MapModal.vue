...
const facilityImage = computed(() => {
  ...
  const type = props.facility.type;

  if (type === "bikeStation") return "/roadMapImage/bike.jpg";
  if (type === "수리시설") return "/roadMapImage/repair.jpg";
  if (type === "공기주입기") return "/roadMapImage/airpump.jpg";
  if (type === "보관소") return "/roadMapImage/rack.jpg";

  return "/roadMapImage/bike.jpg";
});
...
<template>
  <div class="relative">
    <button 
  @click="toggleModal" 
  class="w-12 h-12 bg-black1 text-black1 dark:bg-black10 rounded-full 
         absolute top-8 left-14 shadow-[4px_4px_5px_rgba(0,0,0,0.4)] flex items-center justify-center
         hover:scale-110 transition-transform duration-200">
      <span v-if="riderModal">
    ...
    <div v-if="riderModal" 
     class="absolute pb-5 w-[500px] h-[700px] bg-black1 rounded-3xl dark:bg-black9 
            left-12 top-28 shadow-[15px_15px_7px_rgba(0,0,0,0.3)] overflow-y-auto">
      <div class="flex flex-col h-full">
      <img :src="facilityImage" 
        ...
        <div class="flex-2 mt-5 text-sm text-left ml-6 text-black10 dark:text-black1">
          ...
          <div v-if="props.facility.type === 'bikeStation'">
            <div class="flex justify-between w-[90%] ">
          ...
          <div v-else>
            <div class="flex justify-between w-[90%]">
              ...
              <p class="flex-1 font-medium">
                <span v-if="props.facility.useYn === '사용'">가능</span>
                <span v-else>❌</span>
              </p>
            </div>
          </div>
        ...
        <div class="flex items-center space-x-4 mb-48 ml-5">
          ...
          <button class="bg-black10 text-white p-2 w-20 rounded-lg 
                  hover:bg-black7 hover:scale-110 
                  transition-transform duration-200">등록</button>
        </div>
      </div>
    </div>
  </div>
