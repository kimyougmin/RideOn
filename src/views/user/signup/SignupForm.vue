<template>
    ...
    <div class="w-[428px] space-y-[32px]">
      <div>
        ...
        <input
          v-model="email"
          type="email"
          class="w-full h-[48px] p-3 border dark:border-black1 rounded text-base bg-transparent focus:border-primaryRed outline-none
          dark:placeholder:text-black1 dark:text-black1"
  ...
  variant="outlined"
  class="w-1/3"
  @update:modelValue="updateBirth('year', $event.value)"
/>
<v-combobox 
  v-model="birthMonth"
  label="월"
  ...
  variant="outlined"
  class="w-1/3"
  @update:modelValue="updateBirth('month', $event.value)"
/>
<v-combobox 
  v-model="birthDay"
  label="일"
  ...
  variant="outlined"
  class="w-1/3"
  @update:modelValue="updateBirth('day', $event.value)"
/>
...
const handleSignup = async () => {
  ...
  if (!emailError.value && !passwordError.value && !confirmPasswordError.value && !nameError.value && !birthError.value && !phoneError.value) {
    try {
      const birthDate = `${birthYear.value}-${birthMonth.value}-${birthDay.value}` // 생년월일을 YYYY-MM-DD 형식으로 변환
